...
.DESCRIPTION
    ...
    Почему Conan вызывается условно: в этом репозитории conanfile.py есть, и
    без него не найдётся GTest, но соседние репозитории собираются одним CMake.
    Проверка наличия файла оставлена, чтобы скрипт остался одинаковым во всех.
...
# --------------------------------------------------------------------------
# 2. Чистка — до Conan, а не после
# --------------------------------------------------------------------------
# Каталог сборки одновременно является --output-folder для Conan: там лежит
# conan_toolchain.cmake, на который ссылается пресет. Если чистить после
# conan install, пресет останется без toolchain-файла.
...
if ($conanRecipeFiles) {
    ...
    # Два --conf здесь не косметика:
    #   generator=Ninja — без него Conan считает, что сборка пойдёт через
    #     "Visual Studio 17 2022", и кладёт в toolchain CMAKE_GENERATOR_PLATFORM
    #     с CMAKE_GENERATOR_TOOLSET. Ninja такие переменные не принимает и
    #     configure падает на "does not support platform specification";
    #   user_presets= (пусто) — иначе Conan кладёт в корень репозитория
    #     CMakeUserPresets.json со ссылкой внутрь каталога сборки, и после
    #     -Clean любой вызов `cmake --preset` падает на несуществующем include.
    Invoke-CheckedNativeCommand -Executable 'conan' `
    ...
    Write-Host "    проект собирается одним CMake."
}
...
Write-Host ""
Write-Host "Точка входа проекта — $((Join-Path $buildDirectory 'BookDB.exe'))"
